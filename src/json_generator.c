                        ...
                        json_gen_flush_cb_t flush_cb, void *priv, int float_precision)
{
    ...
    jstr->free_ptr = buf;
    jstr->priv = priv;
    jstr->float_precision = float_precision;
}
...
static int json_gen_set_float_ex(json_gen_str_t *jstr, float val, const char *format)
{
    jstr->comma_req = true;
    char str[MAX_FLOAT_IN_STR];
    snprintf(str, MAX_FLOAT_IN_STR, format, val);
    return json_gen_add_to_str(jstr, str);
}
...
int json_gen_obj_set_float_ex(json_gen_str_t *jstr, const char *name, float val, const char *format)
{
    json_gen_handle_comma(jstr);
    json_gen_handle_name(jstr, name);
    return json_gen_set_float_ex(jstr, val, format);
}
